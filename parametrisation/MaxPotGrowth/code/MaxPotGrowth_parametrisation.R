# Script that give the MaxPotGrowth parameter value of the species with the climate of the LDTRF:
# -Find climatics variable in PEP and LDTRF
# -Open the data of Anthony, 
# -Compute the growth climate relationship
# -Compute growth for the initial conditions --> PEP and LDTRF


rm(list=ls())


setwd("~/PhD/Chap1/Github/parametrisation/MaxPotGrowth/code")
...
# LDTRF climate data
LDTRF_clim_brut <- read.csv("../data/LDTRF_climateNA_1970-2019MSY.csv")
colnames(LDTRF_clim_brut)[2] <- "ID1"

LDTRF_clim <- LDTRF_clim_brut %>% 
  mutate(CMI_JJA=rowSums(LDTRF_clim_brut[c("CMI06",	"CMI07",	"CMI08")])) %>%
...
LDTRF_clim_spatial <- bind_rows(mutate(LDTRF_clim_brut, ID1="H1760"),
                               mutate(LDTRF_clim_brut, ID1="H1823"),
                               mutate(LDTRF_clim_brut, ID1="H1847"),
                               mutate(LDTRF_clim_brut, ID1="H1870"),
                               mutate(LDTRF_clim_brut, ID1="H1916"),
                               mutate(LDTRF_clim_brut, ID1="H1944"))

spatial_clim <- bind_rows(LDTRF_clim_spatial, RESEF_clim)
...
# Merge LDTRF RESEF and PEP to compute in the same loop.
LDTRF_clim$ID1 <- "LDTRF"
PEP_clim$ID1 <- as.character(PEP_clim$ID1)

LDTRF_PEP_clim <- rbind(spatial_clim, PEP_clim, LDTRF_clim)
...
# coefficients from RESEF and LDTRF data set
PSP_norm <- PSP  %>%
...
LDTRF_PEP_clim_norm <- LDTRF_PEP_clim %>%
...
LDTRF_PEP_growth <- data.frame(ID=as.character(), 
...
for(i in code_esp$code){
  ...
  #Prédiction avec les données des PEP et LDTRF
  X0 <- (log(coef_scale_sp$X0) - coef_scale_sp$m) / coef_scale_sp$sd #remplacer par X0
  LDTRF_PEP_clim_norm$dbh_effect <- coef_scale_sp$coef_lin * X0 + coef_scale_sp$coef_quad * X0^2
  fit <- fitted(brq, newdata=LDTRF_PEP_clim_norm, dpar="mu")
  
  LDTRF_PEP_growth <- rbind(LDTRF_PEP_growth,
                            data.frame(ID=LDTRF_PEP_clim_norm$ID1, 
...
spatial_growth <- LDTRF_PEP_growth %>% 
...
growth_mod <- readRDS("../../growth_altering_effect/results/paramater_value/potential_growth_resef_ldtrf.rds") 
...
# with PSP data set.  
LDTRF_PEP_growth <- LDTRF_PEP_growth %>% 
  filter(!ID %in% spatial_clim$ID1)
...
ggplot(data=LDTRF_PEP_growth, aes(x=species, y=growth_no_bias)) + geom_boxplot() + ylim(0, NA) +
  ylab("MaxPotGrowth (in cm)")
ggplot(data=LDTRF_PEP_growth, aes(x=species, y=log_growth_no_bias)) + geom_boxplot() +
  ylab("MaxPotGrowth (in cm)")


saveRDS(LDTRF_PEP_growth, file="../results/LDTRF_PEP_MaxPotGrowth.rds")

#Plot of maximum growth of species at DHP=X0
LDTRF_PEP_growth$species <- str_replace(LDTRF_PEP_growth$species, "_", "\n")

png(filename = "../results/MaxPotGrowth.png", width = 650, height = 400)
ggplot(data=filter(LDTRF_PEP_growth, species!="Jack\nPine"), aes(x=species, y=growth_no_biais)) + geom_boxplot() + 
  #geom_point(data=filter(LDTRF_PEP_growth, ID=="LDTRF"), aes(x=species, y=growth15cm)) +
...
LDTRF <- filter(LDTRF_PEP_clim_norm, ID1=="LDTRF") %>% ungroup() %>% select(CMI_JJA, MSP, MAT, DD5) %>% t()
LDTRF <- data.frame(x=as.numeric(LDTRF[,1]), fix_eff=rownames(LDTRF))
LDTRF$fix_eff <-  str_replace(LDTRF$fix_eff, "_", " ")
...
ggplot(data=filter(response_curve_data, species %in% c("Sugar Maple", "Red Maple", "Yellow Birch")), aes(x=x, y=exp(y), ymin=exp(y-se), ymax=exp(y+se))) + 
  ...
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 14),
        strip.text = element_text(size = 12)) +
  geom_vline(data=LDTRF, aes(xintercept=x), linetype="dashed") + 
  ylim(0,NA)
dev.off()