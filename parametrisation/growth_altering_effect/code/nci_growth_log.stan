...
model {
  ...
  partial_mu = interc + sd_pls * r_pls[id_pls] + sd_tree * r_tree[id_tree] +
                b_dbh_lin * ln_dbh + b_dbh_quad * square(ln_dbh) + b_shade * shade;
  // Add sum of NCI contributions to each measurement
  for (i in 1:n_mes) {
    mu[i] = partial_mu[i] - sum(partial_nci[first_ngb[i]:last_ngb[i]]); 
  }  
  
  gr ~ lognormal(mu, sigma);
}
...
generated quantities {
  ...
  
  {
    ...
    for (i in 1:n_mes) {
      mu[i] = partial_mu[i] - sum(partial_nci[first_ngb[i]:last_ngb[i]]); 
      log_lik[i] = lognormal_lpdf(gr[i] | mu[i], sigma);
    }
  }
